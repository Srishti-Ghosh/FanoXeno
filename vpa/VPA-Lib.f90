...
! Check has been performed for several combinations of l, E, and angle range.
! The equality of both the formulae has been confirmed.
! The quantity |(wk2-wk3)/(wk2+wk3)| is typically 1.0x10^(-15) or less.
!
! End of the part for debugging.
!
   80 continue     
! End of scattering angle loop.
!
      return
      end subroutine
      
      subroutine wparam (Lmin,Lmax,lr,ym,  &
    &                     scatang,angwpm,angqpm,angbgd,angmin,angmax,nangstep)
!
! Angle dependent resonance profile parameter w(theta), q(theta), 
! and the background facter z(theta).
! See F. Koike, J. Phys. B2, 741 (1977).                              .
!
      implicit real *8 (a-h,o-z)
      dimension ym(0:100), scatang(0:360), angqpm(0:360), angbgd(0:360), ZL(0:200)
...
! Start scattering angle loop.
      do 80 i = 0, nangstep
        theta = scatang(i)
! For w-parameter and q-parameter.       
        wk2 = 0.0d0
        do 70 ll = 0, 2*Lmax
          wk2 = wk2 + BL(ll) * Pnx(ll,cos(theta))
   70   continue
! w-parameter:
        angwpm(i) = wk2
! q-parameter:        
        angqpm(i) = -1/tan( (1.0d0/2.0d0) * atan2(aimag(wk2),real(wk2)) )
!        
! For background factor z.
        wk7 = 0.0d0
        do 75 ll = 0, 2*Lmax
