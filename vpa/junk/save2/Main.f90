...
!---------------------------------------------------------------------------
      ...
      real*8, dimension(0:360) :: scatang, angdcs,asydcs,angbgd
...
!     Output the angle dependent background.
       open(unit=14,file ='./angbgd.txt',status='unknown')
...
! 
        call wparam(Lmin,Lmax,lr,ym,scatang,angwpm,angbgd,angmin,angmax,nangstep)
        do 70 j = 0, nangstep
...
!
        ...
        do 80 j = 0, nangstep
! Both the following formulae are confirmed to be the same within the accuracy of 1x10^(-15)%.       
...
!         asydcs(j)= (1/(2*ak2))*(real(-angwpm(j)*(eps-(0.0d0,1.0d0))/(eps+(0.0d0,1.0d0)))+angbgd(j)) 
          ...
          write(14,801) E(i)*autev/CM1teV,scatang(j)*180/pi,angbgd(j)*sin(scatang(j))*(1/(2*ak2))      
!   write(6,801)E(i)*autev/CM1teV,scatang(j)*180/pi,(angbgd(j)-abs(angwpm(j)))*sin(scatang(j))*(1/(2*ak2))      
   80   continue
        write(13,802)
        write(14,802)
!  write(6,802)    
