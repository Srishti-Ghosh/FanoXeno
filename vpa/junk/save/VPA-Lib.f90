...
!
      ...
      dimension ym(100), scatang(361), angdcs(361), BL(200)
      complex *16 wk0
      data pi/3.1415926535897932384626433832795d0/
!
! Prepare list of scattering angles in units of radian.
      do 10 i = 0, nangstep
      scatang(i+1) = angmin + (angmax-angmin)/nangstep*i  ! in deg.
      scatang(i+1) = scatang(i+1)*(pi/180)                ! in rad.
   10 continue   
!
...
!     write(6,*) "Note3", lmin,lmax,l2min,l2max,ll,l1      
     ...
     &              *sin(ym(l1+1))*sin(ym(l2+1))*cos(ym(l1+1)-ym(l2+1))
   30       continue
   40     continue
        BL(ll+1) = wk1
   50 continue
...
! Start BB formula loop        
        wk2 = 0.0d0
          do 70 ll = 0, 2*Lmax
            wk2 = wk2 + BL(ll+1) * Pnx(ll,cos(theta))
   70     continue
        wk2 = ( 1 / ak2 ) * wk2
        angdcs(i+1) = wk2
...
!  &             *(1.0d0-exp(2*(0.0d0,1.0d0)*ym(l+1)))*Pnx(l,cos(theta))
!  75     continue
...
!
      implicit real *8 (a-h,o-z)
      dimension ym(100), scatang(361)
      complex *16 wk0,wk1,wk2
      complex *16, dimension(361) :: angwpm, angbgd 
      complex *16, dimension(200) :: BL, ZL
      data pi/3.1415926535897932384626433832795d0/
!
! Prepare list of scattering angles in units of radian.
      do 10 i = 0, nangstep
...
!       write(6,*) l, ym(l+1)
         if ( l .eq. lr ) then
           ...
           wk1 = wk1 + (2*l+1)*(2*lr+1)*cf(l,lr,ll,0,0,0)**2*(1-exp(-2*(0.0d0,1.0d0)*ym(l+1)))
         end if
   40  continue
      BL(ll+1) = wk0 * wk1                  
   50 continue
...
!
      do 55 ll = 0, 2*Lmax
        ...
        do 54 l = Lmin, Lmax
          if ( l .ne. lr ) then
            wk6 = wk6 + 2*(2*l+1)*(2*lr+1)*cf(l,lr,L,0,0,0)**2*sin(ym(l+1))**2
            do 53 lp = Lmin, Lmax
              if ( lp .ne. lr ) then
                wk6 = wk6 + 2*(2*l+1)*(2*lp+1)*cf(l,lp,L,0,0,0)**2  &
   &                      * sin(ym(l+1))*sin(ym(lp+1))*cos(ym(l+1)-ym(lp-1))
              end if
  53        continue
          end if
  54    continue
...
! Start L sum: ll = 0 to 2*Lmax
   ...
   70   continue
        angwpm(i+1) = wk2
!       write(6,*) wk2, angwpm(i+1)
! End of L sum. 
! For background factor
! Start L sum: ll = 0 to 2*Lmax
        wk7 = 0.0d0
        do 75 ll = 0, 2*Lmax
          wk7 = wk7 + ZL(ll+1) * Pnx(ll,cos(theta))
   75   continue
        angbgd(i+1) = wk7   
   80 continue     
      return
